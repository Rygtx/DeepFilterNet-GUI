...
#[no_mangle]
pub unsafe extern "C" fn dfgui_create(
    channels: usize,
    atten_lim_db: f32,
...
#[no_mangle]
pub unsafe extern "C" fn dfgui_free(state: *mut BridgeState) {
    if !state.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn dfgui_get_frame_length(state: *const BridgeState) -> usize {
    state.as_ref().map(|s| s.model.hop_size).unwrap_or(0)
}

#[no_mangle]
pub unsafe extern "C" fn dfgui_get_sample_rate(state: *const BridgeState) -> usize {
    state.as_ref().map(|s| s.model.sr).unwrap_or(0)
}

#[no_mangle]
pub unsafe extern "C" fn dfgui_get_channel_count(state: *const BridgeState) -> usize {
    state.as_ref().map(|s| s.model.ch).unwrap_or(0)
}

#[no_mangle]
pub unsafe extern "C" fn dfgui_set_atten_lim(state: *mut BridgeState, atten_lim_db: f32) {
...
#[no_mangle]
pub unsafe extern "C" fn dfgui_set_post_filter_beta(state: *mut BridgeState, post_filter_beta: f32) {
    if let Some(state) = state.as_mut() {
...
#[no_mangle]
pub unsafe extern "C" fn dfgui_process_frame(
...
#[no_mangle]
pub unsafe extern "C" fn dfgui_resampler_create_fixed_in(
    input_sample_rate: usize,
    output_sample_rate: usize,
    chunk_size_in: usize,
    sub_chunks: usize,
...
#[no_mangle]
pub unsafe extern "C" fn dfgui_resampler_create_fixed_out(
    input_sample_rate: usize,
    output_sample_rate: usize,
    chunk_size_out: usize,
    sub_chunks: usize,
...
#[no_mangle]
pub unsafe extern "C" fn dfgui_resampler_free(state: *mut ResamplerState) {
    if !state.is_null() {
        let _ = Box::from_raw(state);
    }
}
...
#[no_mangle]
pub unsafe extern "C" fn dfgui_resampler_reset(state: *mut ResamplerState) {
    if let Some(state) = state.as_mut() {
        state.reset();
    }
}
...
#[no_mangle]
pub unsafe extern "C" fn dfgui_resampler_get_input_frames_max(state: *const ResamplerState) -> usize {
    state.as_ref().map(|s| s.input_frames_max()).unwrap_or(0)
}

#[no_mangle]
pub unsafe extern "C" fn dfgui_resampler_get_input_frames_next(state: *const ResamplerState) -> usize {
    state.as_ref().map(|s| s.input_frames_next()).unwrap_or(0)
}

#[no_mangle]
pub unsafe extern "C" fn dfgui_resampler_get_output_frames_max(state: *const ResamplerState) -> usize {
    state.as_ref().map(|s| s.output_frames_max()).unwrap_or(0)
}

#[no_mangle]
pub unsafe extern "C" fn dfgui_resampler_get_output_frames_next(state: *const ResamplerState) -> usize {
    state.as_ref().map(|s| s.output_frames_next()).unwrap_or(0)
}

#[no_mangle]
pub unsafe extern "C" fn dfgui_resampler_get_output_delay(state: *const ResamplerState) -> usize {
    state.as_ref().map(|s| s.output_delay()).unwrap_or(0)
}

#[no_mangle]
pub unsafe extern "C" fn dfgui_resampler_process(
